...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            ...
            stft_kwargs=None, fixed_snr=1, **ignored_kwargs):
...
class Specs_SNR(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
        ...
        if format == "default":
            self.clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
            self.noisy_files = sorted(glob(join(data_dir, subset) + '/noisy/*.wav'))
            self.active_rms = join(data_dir, subset) + '/active_rms.txt'
            self.clean_rms = []
            self.noise_rms = []

            with open(self.active_rms, 'r') as f:
                while(True):
                    try:
                        temp = f.readline().split('\t')
                        self.clean_rms.append(float(temp[1]))
                        self.noise_rms.append(float(temp[2]))
                    except:
                        break
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        gpu=True, normalize='noisy', transform_type="exponent", fixed_snr=1, **kwargs
    ):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_set = Specs(data_dir=self.base_dir, subset='train',
                ...
                normalize=self.normalize, fixed_snr=self.fixed_snr, **specs_kwargs)
            self.valid_set = Specs_SNR(data_dir=self.base_dir, subset='valid',
                dummy=self.dummy, shuffle_spec=False, format=self.format,
                normalize=self.normalize, **specs_kwargs)
            self.valid_set_2 = Specs(data_dir=self.base_dir, subset='valid2',
                dummy=self.dummy, shuffle_spec=False, fixed_snr=1, format=self.format,
                normalize=self.normalize, **specs_kwargs)
        if stage == 'test' or stage is None:
            self.test_set = Specs(data_dir=self.base_dir, subset='test',
                dummy=self.dummy, shuffle_spec=False, format=self.format,
                normalize=self.normalize, fixed_snr=1, **specs_kwargs)
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            num_workers=self.num_workers, pin_memory=self.gpu, shuffle=True, drop_last=True
        )

    def val_dataloader(self):
        return DataLoader(
            self.valid_set, batch_size=1,
            num_workers=self.num_workers, pin_memory=self.gpu, shuffle=False, drop_last=True
        )
    
    def val_dataloader_2(self):
        return DataLoader(
            self.valid_set_2, batch_size=self.batch_size,
            num_workers=self.num_workers, pin_memory=self.gpu, shuffle=False, drop_last=True
        )
