...
ResnetBlockDDPM = layerspp.ResnetBlockDDPMpp_snr
ResnetBlockBigGAN = layerspp.ResnetBlockBigGANpp_snr
...
@BackboneRegistry.register("ncsnpp_snr")
class NCSNpp_snr(nn.Module):
        ...
        **unused_kwargs
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch)) # add semb
                in_ch = out_ch
            ...
            if i_level != num_resolutions - 1:
                if resblock_type == 'ddpm':
                    ...
                    modules.append(ResnetBlock(down=True, in_ch=in_ch)) # add semb
        ...
        in_ch = hs_c[-1]
        modules.append(ResnetBlock(in_ch=in_ch)) # add semb
        modules.append(AttnBlock(channels=in_ch))
        modules.append(ResnetBlock(in_ch=in_ch)) # add semb
        ...
        for i_level in reversed(range(num_resolutions)):
            for i_block in range(num_res_blocks + 1):  # +1 blocks in upsampling because of skip connection from combiner (after downsampling)
                out_ch = nf * ch_mult[i_level]
                modules.append(ResnetBlock(in_ch=in_ch + hs_c.pop(), out_ch=out_ch)) # add semb
                in_ch = out_ch
            ...
            if i_level != 0:
                if resblock_type == 'ddpm':
                    ...
                    modules.append(ResnetBlock(in_ch=in_ch, up=True)) # add semb
    ...
    def forward(self, x, time_cond, noise_cond):
        ...
        if self.embedding_type == 'fourier':
            # Gaussian Fourier features embeddings.
            used_sigmas = noise_cond
            temb = modules[m_idx](torch.log(time_cond))
            m_idx += 1
            semb = modules[m_idx](torch.log(noise_cond))
            m_idx += 1
            ...
            timesteps = time_cond
            noisesizes = noise_cond
            used_sigmas = self.sigmas[noise_cond.long()]
            temb = layers.get_timestep_embedding(timesteps, self.nf)
            semb = layers.get_timestep_embedding(noisesizes, self.nf)
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                h = modules[m_idx](hs[-1], temb, semb)
                m_idx += 1
        ...
        h = modules[m_idx](h, temb, semb)  # ResNet block
        m_idx += 1
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb, semb)
                m_idx += 1
            ...
            if i_level != 0:
                if self.resblock_type == 'ddpm':
                    ...
                    h = modules[m_idx](h, temb, semb)  # Upspampling
                    m_idx += 1
