...
if __name__ == '__main__':
    ...
    parser.add_argument("--no_correct_stepsize", dest='correct_stepsize', action="store_false",
                        help="Use correct stepsize timestep[i] - timestep[i+1]")
    parser.add_argument("--modeltype", type=str, choices=["bbed", "sebridge", 'sebridge_v2'], default="bbed")
    ...
    if oracle == True:
        active_rms = join(args.test_dir) + '/active_rms.txt'
        clean_rms = []
        noise_rms = []

        with open(active_rms, 'r') as f:
            while(True):
                try:
                    temp = f.readline().split('\t')
                    clean_rms.append(float(temp[1]))
                    noise_rms.append(float(temp[2]))
                except:
                    break
    ...
    for cnt, noisy_file in tqdm(enumerate(noisy_files)):
        ...
        if oracle == True:
            x_hat = model.enhance(x, y, sampler_type=sampler_type, predictor=predictor, 
                    ...
                    atol=atol, rtol=rtol, timestep_type=timestep_type, correct_stepsize=correct_stepsize, oracle=args.oracle, clean_rms=clean_rms[cnt], noise_rms=noise_rms[cnt])
        else:
            x_hat = model.enhance(x, y, sampler_type=sampler_type, predictor=predictor, 
                    corrector=corrector, corrector_steps=corrector_steps, N=N, snr=snr,
                    atol=atol, rtol=rtol, timestep_type=timestep_type, correct_stepsize=correct_stepsize, oracle=args.oracle, clean_rms=1, noise_rms=1)
        ...
        write(join(target_dir, 'all', filename), x_hat, 16000)
        ...
        print(" avg PESQ: {0:.3f}".format(_pesq / (cnt + 1)))
